...
class OAuthRefreshException(OAuthException):
    ...
    def __init__(self, response=None, error=None, error_description=None,
                 cause=None):
        super().__init__(response)
        self.cause = cause
        self.data = self._load_data()
        self._error = (
            error if error is not None else self.data.get('error')
        )
        self._error_description = (
            error_description
            if error_description is not None
            else self.data.get('error_description')
        )
    ...
    def __str__(self):
        if self.error and self.error_description:
            return (
                f'{self.message}: {self.error} - {self.error_description}'
            )
        if self.error:
